...
- **Writing style**: no em dashes, no parentheses in prose, no AI attribution in own output, no ASCII art or ASCII diagrams of any kind. All visual representations must use Mermaid syntax in fenced code blocks. Natural writing for all external output: vary structure, no parallel enumeration, no bold labels in prose, self-review before posting.
